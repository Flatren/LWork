...
pFunction Get_Funcion_Dll(HMODULE &hLib)
{
	...
	//image->image = p_Function(file,&(image->width),&(image->heigth));
	//
	return p_Function;
}
...
void ClickPaintCircle_(HWND hWnd, char* _is,char ID_Gamer, ConfigINI* config,std::vector<Image*> *images, int win_x, int win_y)
{
	...
	else
	{
		...
		for (int i = 0; i < config->n; i++)
			for (int j = 0; j < config->n; j++)
				if (_is[i*config->n + j])
				{
					HBITMAP hBitmap = CreateBitmap((*images)[_is[i*config->n + j] - 1]->width, (*images)[_is[i*config->n + j] - 1]->heigth, 1, 32, (*images)[_is[i*config->n + j] - 1]->image);
					HDC memBit = CreateCompatibleDC(hdc);
					SelectObject(memBit, hBitmap);
